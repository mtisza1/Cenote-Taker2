...
if [ $PROPHAGE == "False" ] ; then
	...
	FOR_RPSBLAST=$( find * -maxdepth 0 -type f -name "*.no_hmmscan2.fasta" )
	if [ -n "$FOR_RPSBLAST" ] ; then
		MDYT=$( date +"%m-%d-%y---%T" )
		echo "time update: running RPSBLAST on each sequence " $MDYT
		time echo "$FOR_RPSBLAST" | xargs -n 1 -I {} -P $CPU -t rpsblast -evalue 1e-4 -num_descriptions 5 -num_alignments 1 -db ${CENOTE_SCRIPT_DIR}/cdd_rps_db/Cdd -seg yes -query {} -line_length 200 -out {}.rpsb.out ; echo "$(tput setaf 5)RPS-BLAST of "{}" complete.$(tput sgr 0)"
...
virus_seg_fastas=$( find * -maxdepth 0 -type f -name "*_vs[0-9][0-9].fna" )
	...
	for NO_END in $virus_seg_fastas ; do
		if grep -i -q "Caudovir\|Ackermannvir\|Herellevir\|Corticovir\|Levivir\|Tectivir\|crAss-like virus\|CrAssphage\|Cyanophage\|Microvir\microphage\|Siphoviridae\|Myoviridae\|phage\|Podovir\|Halovir\|sphaerolipovir\|pleolipovir\|plasmid\|Inovir\|Ampullavir\|Bicaudavir\|Fusellovir\|Guttavir\|Ligamenvir\|Plasmavir\|Salterprovir\|Cystovir" ${NO_END%.fna}.tax_guide.blastx.out ; then
			...
			if [ "$ENFORCE_START_CODON" == "True" ] ; then
				...
				sed 's/ /@/g' ${NO_END%.fna}.phan.fasta | bioawk -c fastx '{ print }' | awk '{ print ">"$1 ; print $2 }' | sed 's/@/ /g' > ${NO_END%.fna}.phan.sort.fasta
			fi
...
INT_TBL=$( find * -maxdepth 0 -type f -name "*_vs[0-9][0-9].int.tbl" )
if [ -n "$INT_TBL" ] ; then
...
INT2_TBL=$( find * -maxdepth 0 -type f -name "*_vs[0-9][0-9].int2.tbl" )
if [ -n "$INT2_TBL" ] ; then
...
COMB3_TBL=$( find * -maxdepth 0 -type f -name "*_vs[0-9][0-9].comb3.tbl" )
if [ -n "$COMB3_TBL" ] ; then
	...
	# Editing and transferring tbl file and fasta (fsa) files to sequin directory
		...
		if [ -s ${feat_tbl2%.comb3.tbl}.phan.fasta ]; then
			...
			if [ -s ${feat_tbl2%.comb3.tbl}.tax_guide.CELLULAR.out ] ; then
				...
				bioawk -v contig_name="$seq_name1" -v srr_var="$srr_number" -v tax_var="$tax_guess" -v perc_var="$perc_id" -v headername="$fsa_head" -v newname="$file_core" -v source_var="$isolation_source" -v rand_var="$rand_id" -v number_var="$file_numbers" -v date_var="$collection_date" -v metgenome_type_var="$metagenome_type" -v srx_var="$srx_number" -v prjn_var="$bioproject" -v samn_var="$biosample" -v chrom_info="$VIR_HIT" -v molecule_var="$MOLECULE_TYPE" -c fastx '{ print ">" newname " [note=highly similar to sequence "chrom_info "] [note= "contig_name" ; closest viral relative: " tax_var " " perc_var "] [organism=" chrom_info " strain ct" rand_var number_var "] [moltype=genomic "molecule_var"][isolation_source=" source_var "] [isolate=ct" rand_var number_var " ] [country=USA] [collection_date=" date_var "] [metagenome_source=" metgenome_type_var "] [note=genome binned from sequencing reads available in " srx_var "] [topology=linear] [Bioproject=" prjn_var "] [Biosample=" samn_var "] [SRA=" srr_var "] [gcode=1]" ; print $seq }' ${feat_tbl2%.comb3.tbl}.fna > sequin_directory/${feat_tbl2%.comb3.tbl}.fsa ;	
			else
