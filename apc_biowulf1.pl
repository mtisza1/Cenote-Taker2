...
while (@contigs) {
    ...
    $command = "/data/tiszamj/python/bin/lastdb temp.apc temp.apc.fa";  # format lastdb for current contig
    system($command);
    print "running LAST self-alignment ... ";
    $command = "/data/tiszamj/python/bin/lastal -s 1 -x 300 -f 0 -T 1 temp.apc temp.apc.fa > temp.apc.lastal";  # self align current contig
    system($command);
    print "done\n";
