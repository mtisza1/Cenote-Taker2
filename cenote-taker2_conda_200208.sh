...
if [ ! -d "$run_title" ]; then
	...
	mv ${run_title}/ ${run_title}_old_${rand_dir} 
	mkdir "$run_title"
fi 
...
if [ $F_READS == "no_reads" ] ; then
	...
	bbtools pileup in=reads_to_all_contigs_over${circ_length_cutoff}nt.sam out=reads_to_all_contigs_over${circ_length_cutoff}nt.coverage.txt
...
if [ ! -z "$CONTIGS_NON_CIRCULAR" ] ;then
	...
	for NO_END in $CONTIGS_NON_CIRCULAR ; do 
		...
		if  [[ $virus_domain_db = "standard" ]] ; then
			hmmscan --tblout ${NO_END%.fasta}.AA.hmmscan.out --cpu $CPU -E 1e-8 ${CENOTE_SCRIPT_DIR}/hmmscan_DBs/virus_specific_baits_plus_missed6a ${NO_END%.fasta}.AA.sorted.fasta
			hmmscan --tblout ${NO_END%.fasta}.AA.hmmscan_replicate.out --cpu $CPU -E 1e-15 ${CENOTE_SCRIPT_DIR}/hmmscan_DBs/virus_replication_clusters3 ${NO_END%.fasta}.AA.sorted.fasta		
		elif [[ $virus_domain_db = "rna_virus" ]]; then
			hmmscan --tblout ${NO_END%.fasta}.AA.hmmscan.out --cpu $CPU -E 1e-8 ${CENOTE_SCRIPT_DIR}/hmmscan_DBs/rna_virus_rdrp_capsid_hmms1 ${NO_END%.fasta}.AA.sorted.fasta
		elif [[ $virus_domain_db = "all_common" ]]; then
			hmmscan --tblout ${NO_END%.fasta}.AA.hmmscan.out --cpu $CPU -E 1e-8 ${CENOTE_SCRIPT_DIR}/hmmscan_DBs/useful_hmms_baits_and_not2a ${NO_END%.fasta}.AA.sorted.fasta
		else
		...
		if [[ $FOR_PLASMIDS = "True" ]]; then
			...
			echo "no viable option given for FOR_PLASMIDS, keeping plasmids by default"
			grep -v "^#" ${NO_END%.fasta}.AA.hmmscan.out | sed 's/ \+/	/g' | sort -u -k3,3 > ${NO_END%.fasta}.AA.hmmscan.sort.out
			grep -v "^#" ${NO_END%.fasta}.AA.hmmscan_replicate.out | sed 's/ \+/	/g' | sort -u -k3,3 > ${NO_END%.fasta}.AA.hmmscan_replicate.sort.out			
		fi	
...
for nucl_fa in $CIRCLES_AND_ITRS ; do
	if [ -s "${nucl_fa%.fasta}.rotate.AA.sorted.fasta" ]; then
		...
		hmmscan --tblout ${nucl_fa%.fasta}.rotate.AA.hmmscan.out --cpu $CPU -E 1e-6 ${CENOTE_SCRIPT_DIR}/hmmscan_DBs/virus_specific_baits_plus_missed6a ${nucl_fa%.fasta}.rotate.AA.sorted.fasta
		hmmscan --tblout ${nucl_fa%.fasta}.rotate.AA.hmmscan_replicate.out --cpu $CPU -E 1e-15 ${CENOTE_SCRIPT_DIR}/hmmscan_DBs/virus_replication_clusters3 ${nucl_fa%.fasta}.rotate.AA.sorted.fasta
...
DOMAINED_CIRCLES_AND_ITRS=$( ls | grep "[0-9].fasta" | grep -v "trnascan-se2.txt\|no_hmmscan2.fasta" )
...
if [ -s "${nucl_fa%.fasta}.rotate.no_hmmscan2.fasta" ]; then

	echo "$(tput setaf 5)"$nucl_fa" Continuing to RPS-BLAST NCBI CDD domains database for each ORF...$(tput sgr 0)" 
	rpsblast -evalue 1e-4 -num_descriptions 5 -num_threads $CPU -line_length 100 -num_alignments 1 -db ${CENOTE_SCRIPT_DIR}/cdd_rps_db/Cdd -query ${nucl_fa%.fasta}.rotate.no_hmmscan2.fasta -out ${nucl_fa%.fasta}.rotate.AA.rpsblast.out ;
	echo "$(tput setaf 5)RPS-BLAST of "${nucl_fa%.fasta}.rotate.no_hmmscan2.fasta" complete.$(tput sgr 0)"
	echo " "
else
...
if [ ! -z "$LIST_OF_VIRAL_DOMAIN_CONTIGS" ] ;then
	...
	. ${CENOTE_SCRIPT_DIR}/annotation_of_linear_contigs_prune_seg1_200207.sh
fi
