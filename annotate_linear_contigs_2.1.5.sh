...
if [ -n "$LINEAR_HALLMARK_CONTIGS" ] ; then
	...
	if [ -s LIN_seqs_for_phanotate.txt ] ; then
		...
		cat LIN_seqs_for_phanotate.txt | sed 's/.fna//g ; s/\.\///g' | xargs -n 1 -I {} -P $CPU phanotate.py -f fasta -o {}.phan.fasta {}.fna
...
if [ -n "$LIN_SORT_AAs" ] ; then
	...
	if [ -n "$DTR_AA_FOR_HMM2" ] ; then
		...
		if [ $TOTAL_AA_SEQS -ge 1 ] ; then 
			...
			echo "$SPLIT_LIN_HMM2" | sed 's/.fasta//g' | xargs -n 1 -I {} -P $CPU -t hmmscan --tblout {}.AA.hmmscan2.out --cpu 1 -E 1e-8 --noali ${CENOTE_DBS}/hmmscan_DBs/useful_hmms_baits_and_not2a {}.fasta >/dev/null 2>&1
...
if [ -n "$PROTEIN_NO_HMMSCAN2" ]; then
	...
	if [ $TOTAL_AA_SEQS -ge 1 ] ; then 
		...
		echo "$SPLIT_LIN_AA_RPS" | sed 's/.fasta//g' | xargs -n 1 -I {} -P $CPU -t rpsblast -evalue 1e-4 -num_descriptions 5 -num_alignments 1 -db ${CENOTE_DBS}/cdd_rps_db/Cdd -seg yes -query {}.fasta -line_length 200 -out {}.rpsb.out >/dev/null 2>&1
