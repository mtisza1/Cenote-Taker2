...
echo "$(tput setaf 5) HMMSCAN with expanded viral DB non-circular/non-ITR contigs with viral domains $(tput sgr 0)"
...
for dark_orf in $dark_orf_list ; do
	if  [[ $HHSUITE_TOOL = "-hhsearch" ]] ; then
		...
		cat ${dark_orf%.for_hhpred.fasta}.out.hhr >> ${dark_orf%.rotate*.for_hhpred.fasta}.out_all.hhr ;
...
echo "$(tput setaf 3) FINISHED ANNOTATING CONTIGS WITHOUT CIRCULARITY OR ITRS BUT WITH AT LEAST 1 VIRAL DOMAIN $(tput sgr 0)"
