...
if [ -n "$COMB3_TBL" ] ; then
	for comb3 in $COMB3_TBL ; do
		...
		if grep -q "[0-9]	tRNA" $comb3 ; then
			grep "[0-9]	tRNA" $comb3 | awk -v name="${comb3%.comb3.tbl}" '{OFS="\t"}{FS="\t"}{ if ($1<$2) {print name, $1, $2} else {print name, $2, $1}}' > ${comb3%.comb3.tbl}.tRNA.bed
		fi
		if grep -q "[0-9]	CDS" $comb3 ; then
			grep "[0-9]	CDS" $comb3 | awk -v name="${comb3%.comb3.tbl}" '{OFS="\t"}{FS="\t"}{ if ($1<$2) {print name, $1, $2} else {print name, $2, $1}}' > ${comb3%.comb3.tbl}.CDS.bed
		fi
